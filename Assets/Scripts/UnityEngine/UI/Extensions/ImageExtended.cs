...
namespace UnityEngine.UI.Extensions
{
	...
	public class ImageExtended : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter
	{
		...
		private void GenerateSlicedSprite(List<UIVertex> vbo)
		{
			...
			for (int i = 0; i < 4; i++)
			{
				s_VertScratch[(4 - i / 2) % 4][i % 2] = a[(i + num) % 4];
				s_VertScratch[1 + i / 2][i % 2] = a2[(i + num) % 4];
			}
			for (int j = 2; j < 4; j++)
			{
		...
		private Vector4 GetAdjustedBorders(Vector4 border, Rect rect)
		{
			for (int i = 0; i <= 1; i++)
			{
				...
				if (num2 < num && num != 0f)
				{
					float num3 = num2 / num;
					int index;
					border[index = i] = border[index] * num3;
					int index2;
					border[index2 = i + 2] = border[index2] * num3;
				}
			}
			return border;
		}
