...
public class TripleBullet : MonoBehaviour
{
	...
	public void Initialize(float angleFix, float forceFix, EnemyTankController etc, float bulletDamage)
	{
		...
		Quaternion rotation = Quaternion.Euler(0f, 0f, 90f + num + UnityEngine.Random.Range((0f - angleFix) / 2f, angleFix / 2f));
		...
		base.transform.rotation = ((!playersBullet) ? etc.TankTurret.transform.rotation : GameplayCommons.Instance.playersTankController.TankTurret.transform.rotation);
		rb = GetComponent<Rigidbody2D>();
		float num3 = 0.26f + UnityEngine.Random.Range((0f - forceFix) / 2f, forceFix / 2f);
		rb.AddForce(vector.normalized * num3, ForceMode2D.Impulse);
